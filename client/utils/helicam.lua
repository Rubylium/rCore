...
function CanOpenHeliCam()
    if IsPlayerInPolmav() then
        Citizen.CreateThread(function()
            helicam = true
            while IsPlayerInPolmav() do
                Citizen.Wait(1)
                local heli = GetVehiclePedIsIn(pPed)
                ...
                if IsHeliHighEnough(heli) then
                    ...
                    if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
                        ...
                        if GetPedInVehicleSeat(heli, 1) == pPed or GetPedInVehicleSeat(heli, 2) == pPed then
                            PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                ...
                if helicam then
                    ...
                    while helicam and not IsEntityDead(pPed) and (GetVehiclePedIsIn(pPed) == heli) and IsHeliHighEnough(heli) do
                        if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
                            PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                            helicam = false
                        end
